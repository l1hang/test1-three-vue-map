...
export default {
  name: '3dMap30',
  setup() {
    let baseEarth = null; //baseEarth是一个实例,用于存储当前地球实例,方便后续操作,比如销毁,重置

    // 重置
    const resize = () => {
      //resize方法,用于监听窗口变化,并且重置地球
      baseEarth.resize();
    };

    const { requestData } = useFileLoader(); //useFileLoader是一个自定义hook,用于加载文件，这里用于加载四川省的geojson数据
    const { transfromGeoJSON } = useConversionStandardData(); //useConversionStandardData是一个自定义hook,用于转换geojson数据,这里用于转换四川省的geojson数据
    const { getBoundingBox } = useCoord(); //useCoord是一个自定义hook,用于获取地图的边界,这里用于获取四川省的边界
    const { createCountryFlatLine } = useCountry(); //useCountry是一个自定义hook,用于创建国家的边界,这里用于创建四川省的边界
    const { initCSS2DRender, create2DTag } = useCSS2DRender(); //useCSS2DRender是一个自定义hook,用于创建css2d渲染,这里用于创建标签,标签用于显示省份的名称
    const { createLightPillar } = useMapMarkedLightPillar({
      //useMapMarkedLightPillar是一个自定义hook,用于创建光柱,这里用于创建省份的光柱,光柱用于标识省份的位置
      scaleFactor: 1.2, //光柱的高度
    });
    // 序列帧
    const { createSequenceFrame } = useSequenceFrameAnimate(); //useSequenceFrameAnimate是一个自定义hook,用于创建序列帧,这里用于创建粒子

    const texture = new THREE.TextureLoader(); //创建纹理加载器,textrue用于加载图片,这里用于加载地图的纹理
    const textureMap = texture.load('/data/map/gz-map.jpg'); //textureMap是地图的纹理,这里用于加载地图的纹理
    const texturefxMap = texture.load('/data/map/gz-map-fx.jpg'); //texturefxMap是地图的特效纹理,这里用于加载地图的特效纹理
    const rotatingApertureTexture = texture.load(
      //rotatingApertureTexture是旋转光圈的纹理,这里用于加载旋转光圈的纹理
    ...
    onMounted(async () => {
      ...
      baseEarth = new CurrentEarth({
        ...
        controls: {
          ...
          maxPolarAngle: (Math.PI / 2) * 0.98, //最大仰角
        },
      });
      baseEarth.run();
